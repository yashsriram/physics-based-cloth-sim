...
public class WaterSimulation extends PApplet {
    ...
    public void setup() {
        ...
        waterColumns = new ArrayList<WaterColumn>();
    ...
    void updateLoop() {
    	...
    	for(int i=0; i<numColumns-1; i++) {
	    	...
	    	w1.midpointMomentum -= (dt/2f)*
					    		  ( sq(w2.momentum)/w2.height + 0.5f*g*sq(w2.height)
					    		   - sq(w1.momentum)/w1.height - 0.5f*g*sq(w1.height)
					    		  )/dz; 
    	}
    	float damp = 0.1f;
    	for(int i=0; i<numColumns-2; i++) {
	    	...
	    	w2.momentum -= dt*(damp*w2.momentum+ sq(w2.midpointMomentum)/w2.midpointHeight + .5*g*sq(w2.midpointHeight)
	    				 - sq(w1.midpointMomentum)/w1.midpointHeight - .5*g*sq(w1.midpointHeight))/dz;
    	}
    ...
    static public void main(String[] passedArgs) {
        String[] appletArgs = new String[]{"demos.WaterSimulation"};
        if (passedArgs != null) {
